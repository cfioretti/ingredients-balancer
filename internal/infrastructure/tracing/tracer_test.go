...
func TestDefaultTracerConfig(t *testing.T) {
	...
	assert.Equal(t, "ingredients-balancer", config.ServiceName)
...
func TestTracerConfigWithEnvironmentVariables(t *testing.T) {
	os.Setenv("OTEL_SERVICE_NAME", "test-ingredients-balancer")
	...
	assert.Equal(t, "test-ingredients-balancer", config.ServiceName)
	assert.Equal(t, "test-endpoint:4318", config.JaegerEndpoint)
}

func TestNewTracerProvider(t *testing.T) {
	config := &TracerConfig{
		ServiceName:        "test-ingredients-balancer",
...
func TestGlobalTracerFunctions(t *testing.T) {
	...
	config := DefaultTracerConfig()
	config.ServiceName = "test-global-ingredients-balancer"
